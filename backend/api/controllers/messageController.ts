...
export async function getOneMessage(
  ...
  const message = await PostgresConnection.runQuery(
...
export async function createMessage(
  ...
  const created = await PostgresConnection.runQuery(
    `SELECT * FROM messages WHERE id = '${newMsg.id}'`
  );
  reply
    .status(201)
...
export async function updateOneMessage(
  ...
  const updatedMessage = await PostgresConnection.runQuery(
    `SELECT * FROM messages WHERE id = '${id}'`
  );
